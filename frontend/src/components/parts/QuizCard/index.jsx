...
const QuizCard = ({ id, title, description, thumbnail }) => {
  return (
    <div className="flex flex-col items-center bg-slate-600 gap-5 p-10 rounded-2xl w-2/5 ">
      ...
      <div className="flex gap-5">
        <Link href={`/quiz/${id}/questions/1`}>
          <button className="bg-slate-300 px-5 py-2 rounded-lg">Play</button>
        </Link>
        <Link href={`/edit/${id}`}>
          <button className="bg-yellow-300 px-5 py-2 rounded-lg">edit</button>
        </Link>
      </div>
    </div>
  );
